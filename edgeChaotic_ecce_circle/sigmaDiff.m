...
sigmaDiffChaotic = zeros(length(EE),length(Theta),length(Xi));
...
for NN=1:1:length(EE)
    ...
    for xiIdx = 1:1:length(Xi)
        ...
        for thetaIdx = 1:1:length(Theta)
            ...
            sigmaDiffChaotic(NN,thetaIdx,xiIdx) = abs(a0row*TM*thetacol)^2*2/pi/k_0;
        end
    end
    disp(NN/length(EE))
end 
...
save data_sigmaDiff
% figure()
% plot(Theta,sigmaDiffChaotic(1,:))
% sigmaTotal = sum(sigmaDiffChaotic(:,:,1),2)*(Theta(2)-Theta(1));
% OneMCosTheta = 1 - cos(repmat(Theta,length(EE),1));
% sigmaTr = sum(OneMCosTheta.*sigmaDiffChaotic(:,:,1),2)*(Theta(2)-Theta(1));
% figure()
% plot(EE, sigmaTotal)
% figure()
% plot(EE, sigmaTr)
